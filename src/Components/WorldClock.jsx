...
function TimeDisplay() {
  const [currentTimes, setCurrentTimes] = useState(
    countries.map((country) => ({
      name: country.name,
      time: dayjs().tz(country.timezone),
    }))
  );

  const [selectedCountry, setSelectedCountry] = useState(countries[0].name); // Initialize with the name of the first country
  ...
  const selectedCountryTime = currentTimes.find(
    (countryTime) => countryTime.name === selectedCountry
  );
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      ...
      >
        <Typography variant="h4" sx={{ mb: 2 }}>
          Select Country to see the local time
        </Typography>
        <Select
        ...
        >
          {countries.map((country) => (
            <MenuItem key={country.timezone} value={country.name}>
              {country.name}
            </MenuItem>
          ))}
        </Select>
        ...
        >
          ...
          {selectedCountryTime && (
            <Box sx={{ mb: 2, textAlign: "center" }}>
              <Typography variant="h6">{selectedCountryTime.name}</Typography>
              <Typography variant="h5">
                {selectedCountryTime.time.format(
